...
void VertexArray::linkAttrib(GLuint layout, GLuint count, GLboolean normalize, GLenum type, GLsizei stride, void* offset)
{
    glVertexAttribPointer(layout, count, type, normalize, stride, offset);
    glEnableVertexAttribArray(layout);
};
...
void VertexArray::linkLayout(VertexLayout& layout) {
    GLuint i = 0;
    GLsizei stride = layout.calculateStride();
    void* offset = (void*) 0;
    for(auto it = layout.getLayout().begin(); it != layout.getLayout().end(); it++) {
        ...
        i++;
        offset = (void*)((GLbyte*)offset + ((size_t)it->count * (size_t)VertexLayout::sizeOfElementType(it->type)));
    }
}
